...
function SpellShot() {
  ...
  useEffect(() => {
    ...
    return () => {
      document.removeEventListener("keydown", moveblockshoot);
    };
  }, []);
  ...
  function positionthings() {
    ...
    function redo(ii) {
      ...
      dave = [
        ...dave,
        { xx: randomnums, yy: randomnumh, velx: rnvelx, vely: rnvely },
      ];
  ...
  useEffect(() => {
    ...
    setStartPos((prevPos) =>
      startpos.map((entry) => {
        if (entry["xx"] > 785 || entry["xx"] < 15) {
          return {
            xx: entry["xx"] - entry["velx"],
            yy: entry["yy"] - entry["vely"],
            velx: -entry["velx"],
            vely: entry["vely"],
          };
        } else if (entry["yy"] > 485 || entry["yy"] < 15) {
          return {
            xx: entry["xx"] - entry["velx"],
            yy: entry["yy"] - entry["vely"],
            velx: entry["velx"],
    ...
    if (shooting) {
      ...
      for (zz = 0; zz < wordy.length; zz++) {
        if (wordy.length === davetwo.length) {
          if (
            davetwo[zz]["xx"] + 20 > shotpos + 25 &&
            shotpos + 25 > davetwo[zz]["xx"] - 20 &&
            davetwo[zz]["yy"] + 20 > shotY &&
            shotY > davetwo[zz]["yy"] - 20
          ) {
            setShooting(false);
