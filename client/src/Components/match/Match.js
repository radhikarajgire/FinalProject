...
function Match(){
    ...
    useEffect(() => {
        ...
        for(i=0; i<8; i++){
            ...
            contextRef.current.rect(sammy['xx'], sammy['yy'], sammy['leng'], sammy['hei']);
    ...
    function positionthings(){
        ...
        setTextPos([])
        function redo(a, b){
            const long = Math.floor(contextRef.current.measureText(customElements[a][b]).width);
            ...
            setPos(before=>[...before, {xx: randomnums-5, yy: randomnumh, leng: long+10, hei: 30}])
    ...
    function startMoving({nativeEvent}){
        ...
        for(i=0; i<pos.length; i++){
            if(pos[i]['xx']<offsetX && offsetX<(pos[i]['xx']+pos[i]['leng']) && pos[i]['yy']<offsetY && offsetY<(pos[i]['yy']+pos[i]['hei'])){
                setFound(true)
                setMovingElement(i)
            }
    ...
    function moveBy({nativeEvent}){
        ...
        contextRef.current.fillText( textpos[i], fish['xx']+5, fish['yy']+20)
        contextRef.current.beginPath();
        contextRef.current.lineWidth="4";
        contextRef.current.strokeStyle="blue";
        contextRef.current.rect(fish['xx'], fish['yy'], fish['leng'], fish['hei']);
        contextRef.current.stroke();}
         }}       
    ...
    <div className={Styles.holder}>
        <div className={Styles.header}>  
            ...
            <canvas onMouseDown={(e)=>startMoving(e)} onMouseMove={(e)=>moveBy(e)} onMouseUp={(e)=>finishMove(e)} ref={canvasRef} className={Styles.canvas} width="800px" height="500px"></canvas>
        </div>
    </div>
    )
